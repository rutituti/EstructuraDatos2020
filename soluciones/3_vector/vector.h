...
template <class T> Vector<T>::Vector(unsigned int numberOfElements) throw (RangeError, OutOfMemory) {
...
template <class T> Vector<T>::Vector(const Vector<T> &source) throw (OutOfMemory) {
	size = source.size;
	data = new T[size];
	if (data == 0) {
		throw OutOfMemory();
...
template <class T> unsigned int Vector<T>::resize(unsigned int newSize) throw (RangeError, OutOfMemory) {
	if (newSize == 0) {
		throw RangeError();
	}
